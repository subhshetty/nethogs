...
#                dict(name="install basic packages", action="> {{ ansible_pkg_mgr }} name={{ item }} state=present update_cache=yes", with_items="nethogs, iptraf-ng"),
                dict(name="Copy and Execute the script", script="/root/final2.sh", register="result", ignore_errors="yes", args=dict( executable="/bin/bash")),
                dict(name="clear ssh.log",  local_action="shell > /root/ssh.log"),
                dict(name="Show output", action=dict(module="debug", args=dict(msg='{{result.stdout_lines}}'))),
                dict(name="store output", local_action=dict(module="lineinfile", args=dict(line="{{result.stdout_lines}}", dest="/root/ssh.log")))
        ]
       )
